...
class VitonHDDataset(data.Dataset):
    def __init__(
        ...
        with open(
            os.path.join(
                dataroot_path, phase, "vitonhd_" + phase + "_tagged.json"
            ),
            "r",
        ) as file1:
    ...
    def __getitem__(self, index):
        ...
        densepose_map = Image.open(
            os.path.join(
                self.dataroot, self.phase, "image-densepose", densepose_name
            )
        )
        ...
        cloth_trim = self.clip_processor(
...
def parse_args():
    ...
    parser.add_argument(
        "--noise_offset", type=float, default=None, help="noise offset"
    )
    parser.add_argument(
        "--use_8bit_adam",
        action="store_true",
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default="/home/omnious/workspace/yisol/Dataset/VITON-HD/zalando",
        help="For distributed training: local_rank",
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with (
                ...
                accelerator.accumulate(image_proj_model),
            ):
                ...
                add_time_ids = torch.cat(
                    [
                        compute_time_ids((args.height, args.height))
                        for i in range(bsz)
                    ]
                )
